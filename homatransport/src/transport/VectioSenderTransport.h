...
#ifndef __HOMATRANSPORT_VECTIOSENDERTRANSPORT_H_
#define __HOMATRANSPORT_VECTIOSENDERTRANSPORT_H_
...
class VectioSenderTransport : public cSimpleModule
{
  public:

  public:
    VectioSenderTransport();
    ~VectioSenderTransport();
  ...
  protected:
    ...
    class InboundMsg
    {
      ...
      public:
        explicit InboundMsg();
        explicit InboundMsg(HomaPkt* rxPkt, VectioSenderTransport* transport);
        ~InboundMsg();
      ...
      public:
        ...
        VectioSenderTransport* transport;
        int bytesGranted = -1;
        int bytesInFlight = -1;
  ...
  protected:
    ...
    int currentSenderInFlightBytes = 0;

    typedef std::map<inet::L3Address,int> ReceiverInFlightBytes;
    ReceiverInFlightBytes receiverInFlightBytes;
    ...
    typedef std::map<inet::L3Address,simtime_t> LastHeardFromReceiver;
    LastHeardFromReceiver lastHeardFromReceiver;

    typedef std::map<inet::L3Address,simtime_t> LastGrantSentToReceiver;
    LastGrantSentToReceiver lastGrantSentToReceiver;

    typedef std::map<inet::L3Address,std::set<uint64_t>> GrantedMsgsPerReceiver;
    GrantedMsgsPerReceiver grantedMsgsPerReceiver;
    ...
    int maxWindSize;
    int minWindSize;
    typedef std::map<inet::L3Address, double> RttPerReceiver;
    RttPerReceiver currRttPerReceiver;
    RttPerReceiver targetDelayPerReceiver;
    typedef std::map<inet::L3Address, int> WindPerDest;
    WindPerDest windPerDest;
    ...
    PendingMsgsToSend::iterator lastChosenMsgItr;
